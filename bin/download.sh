...
function dl_cal1(){
   ...
   if [[ ! -z $( cat $1 | grep juelich )  ]]; then 
     ...
     sed 's?srm://lofar-srm.fz-juelich.de:8443?gsiftp://lofar-gridftp.fz-juelich.de:2811?g' $1 | xargs -I{} gfal-copy {} $RUNDIR/Input/ || { echo 'downloading failed' ; exit 21; }
   fi
   ...
   if [[ ! -z $( cat $1 | grep sara )  ]]; then
     ...
     sed 's?srm://srm.grid.sara.nl:8443?gsiftp://gridftp.grid.sara.nl:2811?g' $1 | xargs -I{} gfal-copy {} $RUNDIR/Input/ || { echo 'downloading failed' ; exit 21; }
   fi
   ...
   if [[ ! -z $( cat $1 | grep psnc )  ]]; then
     ...
     sed 's?srm://lta-head.lofar.psnc.pl:8443?gsiftp://gridftp.lofar.psnc.pl:2811?g' $1 | xargs -I{} gfal-copy {} $RUNDIR/Input/ || { echo 'downloading failed' ; exit 21; }
   fi
...
#   sed 's?srm://srm.grid.sara.nl:8443?gsiftp://gridftp.grid.sara.nl:2811?g' $1 | xargs -I{} globus-url-copy -st 30 {} $PWD/Input/ || { echo 'downloading failed' ; exit 21; }
   cd ${RUNDIR}/Input
   globus-url-copy gsiftp://gridftp.grid.sara.nl:2811/pnfs/grid.sara.nl/data/lofar/user/sksp/diskonly/pipelines/SKSP/prefactor_v1.0/pref_cal1/${OBSID}/* ./  || { echo 'downloading failed' ; exit 21;  }
