...
function runtaql(){
    ...
    if  [ ! -z ${SIMG+x} ]; then
        ...
        FREQ=$( singularity exec $SIMG echo "select distinct REF_FREQUENCY from $( ls -d ${RUNDIR}/Input/*${OBSID}*SB* )/SPECTRAL_WINDOW"| singularity exec $SIMG taql | tail -n 1 | head -n 1)
    else
